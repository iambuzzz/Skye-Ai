...
const Chat = () => {
  ...
  useEffect(() => {
    getUserChats().then((res: { chats: any[] }) => {
      const withIds = res.chats.map((c, i) => ({
        ...c,
        id: "msg-" + i,
      }));
      setMessages(withIds);
  ...
  const send = async () => {
    ...
    const withIds = res.chats.map((c: any, i: number) => ({
      ...c,
      id: "msg-" + i,
    }));
    setMessages(withIds);
  ...
  return (
    <Box sx={{ display: "flex", height: "100vh" }}>
      ...
      <Box sx={{ width: "20%", p: 2, background: "#111d27" }}>
        ...
        <Button color="error" fullWidth onClick={deleteUserChats}>
          Clear Conversation
        </Button>
      </Box>
